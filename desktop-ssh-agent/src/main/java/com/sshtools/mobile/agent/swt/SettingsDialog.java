						...
						public void run() {
							try {
								...
								if(SystemUtils.IS_OS_WINDOWS) {
									cmd =  new CommandExecutor("cmd.exe", "/c", "start", "authorize.exe");
								} else if(SystemUtils.IS_OS_MAC_OSX) {
									cmd =  new CommandExecutor("/usr/bin/open", "-a", "Terminal", "authorize");
								} else {
									cmd =  new CommandExecutor("/usr/bin/x-terminal-emulator", "--disable-factory", "-e", "authorize");
								}
								...
								Log.error("Authorization command failed", e);
							}
						}
					}.start();
